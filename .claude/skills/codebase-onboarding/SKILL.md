...
- User asks for a code template, pattern reference, or "how do I create a hook / component / util / API route"
...
**Tech Stack**
Load `.claude/skills/codebase-onboarding/assets/tech-stack-fingerprints.json` and match detected config files and dependencies against its entries to name the framework, language, test runner, CI platform, and database. If a file matches multiple entries, prefer the most specific match.
...
Read `.claude/skills/codebase-onboarding/assets/onboarding-guide-template.md`, fill every `[placeholder]` with detected values, and print the result to the conversation. Do not write it to a file.

#### Output 2: Starter CLAUDE.md

Read `.claude/skills/codebase-onboarding/assets/starter-claude-md.md`, fill every `[placeholder]` with detected values, then write the result to `CLAUDE.md` in the project root.

If `CLAUDE.md` already exists, read it first and enhance it — preserve existing project-specific instructions and clearly call out what was added or changed. Do not use the starter template as a replacement in that case; use it only as a reference for the expected structure.

## Reference Templates

Pre-built code templates live in `.claude/skills/codebase-onboarding/references/`:

| File | Covers |
|------|--------|
| `custom-hook-template.ts` | Data-fetching hooks and stateful logic hooks |
| `utility-template.ts` | Pure functions and localStorage/browser-API utilities |
| `component-template.tsx` | Leaf (atomic) components and composite components |
| `api-route-template.ts` | Next.js App Router GET/POST handlers with caching and fallback |
| `testing-patterns.ts` | All test types: components, hooks, utils, API routes, jest-axe, MSW |

**Serving templates — rules:**

1. **Never show templates during the standard onboarding flow.** The onboarding guide and CLAUDE.md are the only outputs of Phases 1–4.
2. **At the end of the onboarding guide**, add one closing line listing the available templates:
   > Code templates for hooks, components, utilities, and API routes are available — ask to see any of them.
3. **Show a template only when the user explicitly requests it.** Triggers include:
   - "show me the hook template"
   - "how do I create a component / util / API route?"
   - "what's the pattern for…"
   - "give me a starting point for a new hook"
4. **When serving a template**: read the file from the `references/` directory, then display its full contents in a code block. Add a one-sentence summary of which pattern inside the file matches the user's request.
5. **If the user asks for something not yet in references/**, say so explicitly rather than generating an ad-hoc template — this keeps patterns consistent.
...
### Example 4: Show a reference template on request
**User**: "show me the hook template" / "how do I create a new utility?" / "what's the API route pattern?"
**Action**: Read the matching file from `.claude/skills/codebase-onboarding/references/`, display its contents in a code block, add one sentence pointing to the relevant pattern inside
**Output**: Full template displayed — no onboarding guide, no CLAUDE.md update